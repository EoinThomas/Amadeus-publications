---
layout: default
title: "Deep Choice Model Using Pointer Networks for Airline Itinerary Prediction"
year: 2017
---

# Deep Choice Model Using Pointer Networks for Airline Itinerary Prediction

[← Back to Publications](/Amadeus-publications/)

## Publication Details

- **Authors:** Mottini, A.; Acuna-Agost, R.
- **Venue:** ACM SIGKDD 2017
- **Year:** 2017
- **Link:** [ACM DL](https://dl.acm.org/doi/abs/10.1145/3097983.3098005)

## Abstract

*[Abstract to be added - available at the ACM Digital Library link above]*

## Key Contributions

- Application of pointer networks to choice modeling
- Deep learning approach for airline itinerary prediction
- Novel architecture for handling variable-sized choice sets
- Strong empirical results on real-world travel data

## Citation

{% raw %}
```bibtex
@inproceedings{mottini2017deep,
  title={Deep Choice Model Using Pointer Networks for Airline Itinerary Prediction},
  author={Mottini, Alejandro and Acuna-Agost, Rodrigo},
  booktitle={Proceedings of the 23rd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining},
  pages={1575--1583},
  year={2017}
}
```
{% endraw %}

---

[← Back to Publications](/Amadeus-publications/)
